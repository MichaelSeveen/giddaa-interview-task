...
import { TAX_COMMUNITY_CARDS } from "@/data";
import type { CommunityCardData } from "@/data";

function CommunityCard({ card }: { card: CommunityCardData }) {
  return (
    <div className="bg-white w-[413px] h-[241px] rounded-[2.5rem] p-4 flex gap-2.5 shrink-0">
      <div className="flex flex-col gap-2">
        ...
        <p className="text-[0.875rem] leading-5 text-[#6A7282]">{card.likes}</p>
      </div>
...
export function TaxCommunitySection() {
  return (
    <section className="w-full bg-[#FFEED0] py-20">
      <div className="container mx-auto flex flex-col gap-10 overflow-hidden max-w-full">
        <div className="flex flex-col gap-6 px-4 md:px-6 lg:px-20">
          <SectionBadge label="Tax Community" />
          <h2 className="hidden lg:block font-bold text-[2.5rem] leading-17 w-[910px] h-[53px] text-[#001F3F]">
            A Vibrant Community You Can Count On.
          </h2>
          <h2 className="block lg:hidden font-bold text-[2rem] leading-12.5 text-[#001F3F]">
        ...
        <Marquee speed={45}>
          {TAX_COMMUNITY_CARDS.map((card) => (
            <CommunityCard key={card.title} card={card} />
          ))}
        </Marquee>

        <div className="relative max-sm:h-[684px] max-sm:w-[858px] max-sm:overflow-hidden lg:grid lg:place-content-center max-sm:ml-2 lg:aspect-2/1">
